...
// Task 4
// При нажатии b-4 выполняете функцию f4. Функция должна вывести в out-4 значение dog из массива a4. Возвратите это значение.
...
function f4() {
  ...
  for (let i = 0; i < a4.length; i++) {
    for (let j = 0; j < a4[i].length; j++) {
      for (let k = 0; k < a4[i][j].length; k++) {

      }
    }
  }
  out4.innerHTML = a4[3][1][1];
}
...
function f13() {
  let p = 0; //содержится четное или нечетное значение(я попробовал иначе, получилось)
  for ( let i = 0; i < 8; i++) {
    let t = []; //складываем вложенный массивб результат своего рода
    for (let j = 0; j < 8; j++) {
      if (i % 2 == 0) {
        (j % 2 == 0) ? t.push(0) : t.push(1);
      } else {
        (j % 2 != 0) ? t.push(0) : t.push(1);
      }
     }
